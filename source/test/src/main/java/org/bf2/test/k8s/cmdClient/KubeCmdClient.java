...
public interface KubeCmdClient<K extends KubeCmdClient<K>> {
  ...
  /**
   * Execute the given {@code command} in the given {@code container} which is deployed in {@code
   * pod}.
   *
   * @param pod The pod
  ...
  /**
   * Execute the given {@code command}. You can specify if potential failure will thrown the
   * exception or not.
   *
  ...
  /**
   * Wait for the resource with the given {@code name} to be reach the state defined by the
   * predicate.
   *

    ...
    public UpdateControl<ManagedServiceAccountRequest> createOrUpdateResource(ManagedServiceAccountRequest resource,
        ...
        if (resource.getSpec().getReset()) {
            ...
            try {
                ...
                var status = new ManagedServiceAccountRequestStatus("Created", LocalDateTime.now().format(DateTimeFormatter.ISO_LOCAL_DATE_TIME), resource.getSpec().getServiceAccountSecretName());
                ...
                var secret = new SecretBuilder()
                    .editOrNewMetadata()
                        ...
                        .withOwnerReferences(List.of(
                            ...
                            .build()))
                    .endMetadata()
                    .withData(Map.of("client-secret", Base64.getEncoder().encodeToString(serviceAccount.getClientSecret().getBytes(Charset.defaultCharset())), 
                                     "client-id", Base64.getEncoder().encodeToString(serviceAccount.getClientID().getBytes(Charset.defaultCharset()))))
                    .build();
